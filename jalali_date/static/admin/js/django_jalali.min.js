...
!(function () {
  ...
  !(function () {
    ...
    var e = function (t) {
      ...
      var themeObserver;
      var isDarkThemeActive = function () {
        var detectTarget = function (node) {
          return (
            node &&
            (node.classList.contains("scheme-dark") ||
              node.classList.contains("dark") ||
              node.getAttribute("data-theme") === "dark")
          );
        };
        return (
          (darkModeQuery && darkModeQuery.matches) ||
          detectTarget(document.documentElement) ||
          detectTarget(document.body)
        );
      };
      var syncDarkTheme = function () {
        var container = vt._dpContainer;
        if (!container) return;
        var active = isDarkThemeActive();
        container.classList.toggle("dark", active);
        vt.overlayElm && vt.overlayElm.classList.toggle("dark", active);
      };
      var startWatchingTheme = function () {
        if (themeObserver) return;
        var targets = [];
        document.documentElement && targets.push(document.documentElement);
        document.body && targets.push(document.body);
